...
module "boston_security_group" {
  source  = "terraform-aws-modules/security-group/aws//modules/web"
  version = "3.17.0"

  name        = "boston-sg-${local.name_suffix}"
  description = "Security group for web-servers with HTTP ports open within VPC"
  vpc_id      = module.vpc.vpc_id
...
resource "aws_instance" "boston" {
  ...
  vpc_security_group_ids = [module.boston_security_group.this_security_group_id]
...
resource "aws_route53_zone" "primary" {
  name = "cloudmendy.tech"
}

resource "aws_route53_record" "terraform-test" {
  zone_id = aws_route53_zone.primary.zone_id
  name    = "terraform-test.cloudmendy.tech"
  type    = "A"
